...
See `backend/agents/prompts/story.md` for the canonical prompt — the
agent loads it byte-for-byte at import time. Phase 6 evolved the prompt
with an explicit anti-pattern list, voice guidance, and a one-shot
Bommanahalli example. The five mandates are unchanged:

1. Three-beat structure (identity → closeness → call), 50/100/50 words.
2. No candidate or party names; frame around margins and turnout.
3. No invented statistics — if a data point is missing, leave it out.
4. Imagen prompts strictly scenic; no political/religious imagery.
5. TTS voice matches `{language_code}`; neutral and warm, not dramatic.

The prompt also explicitly bans common AI tells:
"In the heart of …", "Did you know …", "Furthermore", "Moreover",
exclamation marks, rhetorical questions, and second-person commands
outside the call beat. Quality is scored on a 5-axis rubric in
`scripts/iterate_narratives.py` — ship gate is each narrative ≥20/25.
